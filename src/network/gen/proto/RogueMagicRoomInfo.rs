// This file is generated by rust-protobuf 3.4.0. Do not edit
// .proto file is parsed by pure
// @generated

// https://github.com/rust-lang/rust-clippy/issues/702
#![allow(unknown_lints)]
#![allow(clippy::all)]

#![allow(unused_attributes)]
#![cfg_attr(rustfmt, rustfmt::skip)]

#![allow(box_pointers)]
#![allow(dead_code)]
#![allow(missing_docs)]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(trivial_casts)]
#![allow(unused_results)]
#![allow(unused_mut)]

//! Generated file from `RogueMagicRoomInfo.proto`

/// Generated files are compatible only with the same version
/// of protobuf runtime.
const _PROTOBUF_VERSION_CHECK: () = ::protobuf::VERSION_3_4_0;

// @@protoc_insertion_point(message:RogueMagicRoomInfo)
#[derive(PartialEq,Clone,Default,Debug)]
pub struct RogueMagicRoomInfo {
    // message fields
    // @@protoc_insertion_point(field:RogueMagicRoomInfo.BPFBJKCFLKH)
    pub BPFBJKCFLKH: u32,
    // @@protoc_insertion_point(field:RogueMagicRoomInfo.status)
    pub status: ::protobuf::EnumOrUnknown<super::RogueMagicRoomStatus::RogueMagicRoomStatus>,
    // @@protoc_insertion_point(field:RogueMagicRoomInfo.room_index)
    pub room_index: u32,
    // @@protoc_insertion_point(field:RogueMagicRoomInfo.room_id)
    pub room_id: u32,
    // special fields
    // @@protoc_insertion_point(special_field:RogueMagicRoomInfo.special_fields)
    pub special_fields: ::protobuf::SpecialFields,
}

impl<'a> ::std::default::Default for &'a RogueMagicRoomInfo {
    fn default() -> &'a RogueMagicRoomInfo {
        <RogueMagicRoomInfo as ::protobuf::Message>::default_instance()
    }
}

impl RogueMagicRoomInfo {
    pub fn new() -> RogueMagicRoomInfo {
        ::std::default::Default::default()
    }

    fn generated_message_descriptor_data() -> ::protobuf::reflect::GeneratedMessageDescriptorData {
        let mut fields = ::std::vec::Vec::with_capacity(4);
        let mut oneofs = ::std::vec::Vec::with_capacity(0);
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "BPFBJKCFLKH",
            |m: &RogueMagicRoomInfo| { &m.BPFBJKCFLKH },
            |m: &mut RogueMagicRoomInfo| { &mut m.BPFBJKCFLKH },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "status",
            |m: &RogueMagicRoomInfo| { &m.status },
            |m: &mut RogueMagicRoomInfo| { &mut m.status },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "room_index",
            |m: &RogueMagicRoomInfo| { &m.room_index },
            |m: &mut RogueMagicRoomInfo| { &mut m.room_index },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "room_id",
            |m: &RogueMagicRoomInfo| { &m.room_id },
            |m: &mut RogueMagicRoomInfo| { &mut m.room_id },
        ));
        ::protobuf::reflect::GeneratedMessageDescriptorData::new_2::<RogueMagicRoomInfo>(
            "RogueMagicRoomInfo",
            fields,
            oneofs,
        )
    }
}

impl ::protobuf::Message for RogueMagicRoomInfo {
    const NAME: &'static str = "RogueMagicRoomInfo";

    fn is_initialized(&self) -> bool {
        true
    }

    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::Result<()> {
        while let Some(tag) = is.read_raw_tag_or_eof()? {
            match tag {
                48 => {
                    self.BPFBJKCFLKH = is.read_uint32()?;
                },
                96 => {
                    self.status = is.read_enum_or_unknown()?;
                },
                104 => {
                    self.room_index = is.read_uint32()?;
                },
                24 => {
                    self.room_id = is.read_uint32()?;
                },
                tag => {
                    ::protobuf::rt::read_unknown_or_skip_group(tag, is, self.special_fields.mut_unknown_fields())?;
                },
            };
        }
        ::std::result::Result::Ok(())
    }

    // Compute sizes of nested messages
    #[allow(unused_variables)]
    fn compute_size(&self) -> u64 {
        let mut my_size = 0;
        if self.BPFBJKCFLKH != 0 {
            my_size += ::protobuf::rt::uint32_size(6, self.BPFBJKCFLKH);
        }
        if self.status != ::protobuf::EnumOrUnknown::new(super::RogueMagicRoomStatus::RogueMagicRoomStatus::ROGUE_MAGIC_ROOM_STATUS_NONE) {
            my_size += ::protobuf::rt::int32_size(12, self.status.value());
        }
        if self.room_index != 0 {
            my_size += ::protobuf::rt::uint32_size(13, self.room_index);
        }
        if self.room_id != 0 {
            my_size += ::protobuf::rt::uint32_size(3, self.room_id);
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.special_fields.unknown_fields());
        self.special_fields.cached_size().set(my_size as u32);
        my_size
    }

    fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::Result<()> {
        if self.BPFBJKCFLKH != 0 {
            os.write_uint32(6, self.BPFBJKCFLKH)?;
        }
        if self.status != ::protobuf::EnumOrUnknown::new(super::RogueMagicRoomStatus::RogueMagicRoomStatus::ROGUE_MAGIC_ROOM_STATUS_NONE) {
            os.write_enum(12, ::protobuf::EnumOrUnknown::value(&self.status))?;
        }
        if self.room_index != 0 {
            os.write_uint32(13, self.room_index)?;
        }
        if self.room_id != 0 {
            os.write_uint32(3, self.room_id)?;
        }
        os.write_unknown_fields(self.special_fields.unknown_fields())?;
        ::std::result::Result::Ok(())
    }

    fn special_fields(&self) -> &::protobuf::SpecialFields {
        &self.special_fields
    }

    fn mut_special_fields(&mut self) -> &mut ::protobuf::SpecialFields {
        &mut self.special_fields
    }

    fn new() -> RogueMagicRoomInfo {
        RogueMagicRoomInfo::new()
    }

    fn clear(&mut self) {
        self.BPFBJKCFLKH = 0;
        self.status = ::protobuf::EnumOrUnknown::new(super::RogueMagicRoomStatus::RogueMagicRoomStatus::ROGUE_MAGIC_ROOM_STATUS_NONE);
        self.room_index = 0;
        self.room_id = 0;
        self.special_fields.clear();
    }

    fn default_instance() -> &'static RogueMagicRoomInfo {
        static instance: RogueMagicRoomInfo = RogueMagicRoomInfo {
            BPFBJKCFLKH: 0,
            status: ::protobuf::EnumOrUnknown::from_i32(0),
            room_index: 0,
            room_id: 0,
            special_fields: ::protobuf::SpecialFields::new(),
        };
        &instance
    }
}

impl ::protobuf::MessageFull for RogueMagicRoomInfo {
    fn descriptor() -> ::protobuf::reflect::MessageDescriptor {
        static descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::MessageDescriptor> = ::protobuf::rt::Lazy::new();
        descriptor.get(|| file_descriptor().message_by_package_relative_name("RogueMagicRoomInfo").unwrap()).clone()
    }
}

impl ::std::fmt::Display for RogueMagicRoomInfo {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::protobuf::text_format::fmt(self, f)
    }
}

impl ::protobuf::reflect::ProtobufValue for RogueMagicRoomInfo {
    type RuntimeType = ::protobuf::reflect::rt::RuntimeTypeMessage<Self>;
}

static file_descriptor_proto_data: &'static [u8] = b"\
    \n\x18RogueMagicRoomInfo.proto\x1a\x1aRogueMagicRoomStatus.proto\"\x9d\
    \x01\n\x12RogueMagicRoomInfo\x12\x20\n\x0bBPFBJKCFLKH\x18\x06\x20\x01(\r\
    R\x0bBPFBJKCFLKH\x12-\n\x06status\x18\x0c\x20\x01(\x0e2\x15.RogueMagicRo\
    omStatusR\x06status\x12\x1d\n\nroom_index\x18\r\x20\x01(\rR\troomIndex\
    \x12\x17\n\x07room_id\x18\x03\x20\x01(\rR\x06roomIdB\x15\n\x13emu.lunarc\
    ore.protob\x06proto3\
";

/// `FileDescriptorProto` object which was a source for this generated file
fn file_descriptor_proto() -> &'static ::protobuf::descriptor::FileDescriptorProto {
    static file_descriptor_proto_lazy: ::protobuf::rt::Lazy<::protobuf::descriptor::FileDescriptorProto> = ::protobuf::rt::Lazy::new();
    file_descriptor_proto_lazy.get(|| {
        ::protobuf::Message::parse_from_bytes(file_descriptor_proto_data).unwrap()
    })
}

/// `FileDescriptor` object which allows dynamic access to files
pub fn file_descriptor() -> &'static ::protobuf::reflect::FileDescriptor {
    static generated_file_descriptor_lazy: ::protobuf::rt::Lazy<::protobuf::reflect::GeneratedFileDescriptor> = ::protobuf::rt::Lazy::new();
    static file_descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::FileDescriptor> = ::protobuf::rt::Lazy::new();
    file_descriptor.get(|| {
        let generated_file_descriptor = generated_file_descriptor_lazy.get(|| {
            let mut deps = ::std::vec::Vec::with_capacity(1);
            deps.push(super::RogueMagicRoomStatus::file_descriptor().clone());
            let mut messages = ::std::vec::Vec::with_capacity(1);
            messages.push(RogueMagicRoomInfo::generated_message_descriptor_data());
            let mut enums = ::std::vec::Vec::with_capacity(0);
            ::protobuf::reflect::GeneratedFileDescriptor::new_generated(
                file_descriptor_proto(),
                deps,
                messages,
                enums,
            )
        });
        ::protobuf::reflect::FileDescriptor::new_generated_2(generated_file_descriptor)
    })
}
