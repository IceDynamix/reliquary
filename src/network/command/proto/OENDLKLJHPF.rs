// This file is generated by rust-protobuf 3.7.1. Do not edit
// .proto file is parsed by pure
// @generated

// https://github.com/rust-lang/rust-clippy/issues/702
#![allow(unknown_lints)]
#![allow(clippy::all)]

#![allow(unused_attributes)]
#![cfg_attr(rustfmt, rustfmt::skip)]

#![allow(dead_code)]
#![allow(missing_docs)]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(trivial_casts)]
#![allow(unused_results)]
#![allow(unused_mut)]

//! Generated file from `OENDLKLJHPF.proto`

/// Generated files are compatible only with the same version
/// of protobuf runtime.
const _PROTOBUF_VERSION_CHECK: () = ::protobuf::VERSION_3_7_1;

// @@protoc_insertion_point(message:OENDLKLJHPF)
#[derive(PartialEq,Clone,Default,Debug)]
pub struct OENDLKLJHPF {
    // message fields
    // @@protoc_insertion_point(field:OENDLKLJHPF.op_type)
    pub op_type: ::protobuf::EnumOrUnknown<super::DDNJKAMIHHI::DDNJKAMIHHI>,
    // @@protoc_insertion_point(field:OENDLKLJHPF.group_id)
    pub group_id: u32,
    // @@protoc_insertion_point(field:OENDLKLJHPF.IIIMCOINAOG)
    pub IIIMCOINAOG: u32,
    // @@protoc_insertion_point(field:OENDLKLJHPF.LKJHCMMPBDL)
    pub LKJHCMMPBDL: u32,
    // special fields
    // @@protoc_insertion_point(special_field:OENDLKLJHPF.special_fields)
    pub special_fields: ::protobuf::SpecialFields,
}

impl<'a> ::std::default::Default for &'a OENDLKLJHPF {
    fn default() -> &'a OENDLKLJHPF {
        <OENDLKLJHPF as ::protobuf::Message>::default_instance()
    }
}

impl OENDLKLJHPF {
    pub fn new() -> OENDLKLJHPF {
        ::std::default::Default::default()
    }

    fn generated_message_descriptor_data() -> ::protobuf::reflect::GeneratedMessageDescriptorData {
        let mut fields = ::std::vec::Vec::with_capacity(4);
        let mut oneofs = ::std::vec::Vec::with_capacity(0);
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "op_type",
            |m: &OENDLKLJHPF| { &m.op_type },
            |m: &mut OENDLKLJHPF| { &mut m.op_type },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "group_id",
            |m: &OENDLKLJHPF| { &m.group_id },
            |m: &mut OENDLKLJHPF| { &mut m.group_id },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "IIIMCOINAOG",
            |m: &OENDLKLJHPF| { &m.IIIMCOINAOG },
            |m: &mut OENDLKLJHPF| { &mut m.IIIMCOINAOG },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "LKJHCMMPBDL",
            |m: &OENDLKLJHPF| { &m.LKJHCMMPBDL },
            |m: &mut OENDLKLJHPF| { &mut m.LKJHCMMPBDL },
        ));
        ::protobuf::reflect::GeneratedMessageDescriptorData::new_2::<OENDLKLJHPF>(
            "OENDLKLJHPF",
            fields,
            oneofs,
        )
    }
}

impl ::protobuf::Message for OENDLKLJHPF {
    const NAME: &'static str = "OENDLKLJHPF";

    fn is_initialized(&self) -> bool {
        true
    }

    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::Result<()> {
        while let Some(tag) = is.read_raw_tag_or_eof()? {
            match tag {
                120 => {
                    self.op_type = is.read_enum_or_unknown()?;
                },
                96 => {
                    self.group_id = is.read_uint32()?;
                },
                32 => {
                    self.IIIMCOINAOG = is.read_uint32()?;
                },
                16 => {
                    self.LKJHCMMPBDL = is.read_uint32()?;
                },
                tag => {
                    ::protobuf::rt::read_unknown_or_skip_group(tag, is, self.special_fields.mut_unknown_fields())?;
                },
            };
        }
        ::std::result::Result::Ok(())
    }

    // Compute sizes of nested messages
    #[allow(unused_variables)]
    fn compute_size(&self) -> u64 {
        let mut my_size = 0;
        if self.op_type != ::protobuf::EnumOrUnknown::new(super::DDNJKAMIHHI::DDNJKAMIHHI::DDNJKAMIHHI_NLCDGIPGFDJ) {
            my_size += ::protobuf::rt::int32_size(15, self.op_type.value());
        }
        if self.group_id != 0 {
            my_size += ::protobuf::rt::uint32_size(12, self.group_id);
        }
        if self.IIIMCOINAOG != 0 {
            my_size += ::protobuf::rt::uint32_size(4, self.IIIMCOINAOG);
        }
        if self.LKJHCMMPBDL != 0 {
            my_size += ::protobuf::rt::uint32_size(2, self.LKJHCMMPBDL);
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.special_fields.unknown_fields());
        self.special_fields.cached_size().set(my_size as u32);
        my_size
    }

    fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::Result<()> {
        if self.op_type != ::protobuf::EnumOrUnknown::new(super::DDNJKAMIHHI::DDNJKAMIHHI::DDNJKAMIHHI_NLCDGIPGFDJ) {
            os.write_enum(15, ::protobuf::EnumOrUnknown::value(&self.op_type))?;
        }
        if self.group_id != 0 {
            os.write_uint32(12, self.group_id)?;
        }
        if self.IIIMCOINAOG != 0 {
            os.write_uint32(4, self.IIIMCOINAOG)?;
        }
        if self.LKJHCMMPBDL != 0 {
            os.write_uint32(2, self.LKJHCMMPBDL)?;
        }
        os.write_unknown_fields(self.special_fields.unknown_fields())?;
        ::std::result::Result::Ok(())
    }

    fn special_fields(&self) -> &::protobuf::SpecialFields {
        &self.special_fields
    }

    fn mut_special_fields(&mut self) -> &mut ::protobuf::SpecialFields {
        &mut self.special_fields
    }

    fn new() -> OENDLKLJHPF {
        OENDLKLJHPF::new()
    }

    fn clear(&mut self) {
        self.op_type = ::protobuf::EnumOrUnknown::new(super::DDNJKAMIHHI::DDNJKAMIHHI::DDNJKAMIHHI_NLCDGIPGFDJ);
        self.group_id = 0;
        self.IIIMCOINAOG = 0;
        self.LKJHCMMPBDL = 0;
        self.special_fields.clear();
    }

    fn default_instance() -> &'static OENDLKLJHPF {
        static instance: OENDLKLJHPF = OENDLKLJHPF {
            op_type: ::protobuf::EnumOrUnknown::from_i32(0),
            group_id: 0,
            IIIMCOINAOG: 0,
            LKJHCMMPBDL: 0,
            special_fields: ::protobuf::SpecialFields::new(),
        };
        &instance
    }
}

impl ::protobuf::MessageFull for OENDLKLJHPF {
    fn descriptor() -> ::protobuf::reflect::MessageDescriptor {
        static descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::MessageDescriptor> = ::protobuf::rt::Lazy::new();
        descriptor.get(|| file_descriptor().message_by_package_relative_name("OENDLKLJHPF").unwrap()).clone()
    }
}

impl ::std::fmt::Display for OENDLKLJHPF {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::protobuf::text_format::fmt(self, f)
    }
}

impl ::protobuf::reflect::ProtobufValue for OENDLKLJHPF {
    type RuntimeType = ::protobuf::reflect::rt::RuntimeTypeMessage<Self>;
}

static file_descriptor_proto_data: &'static [u8] = b"\
    \n\x11OENDLKLJHPF.proto\x1a\x11DDNJKAMIHHI.proto\"\x93\x01\n\x0bOENDLKLJ\
    HPF\x12%\n\x07op_type\x18\x0f\x20\x01(\x0e2\x0c.DDNJKAMIHHIR\x06opType\
    \x12\x19\n\x08group_id\x18\x0c\x20\x01(\rR\x07groupId\x12\x20\n\x0bIIIMC\
    OINAOG\x18\x04\x20\x01(\rR\x0bIIIMCOINAOG\x12\x20\n\x0bLKJHCMMPBDL\x18\
    \x02\x20\x01(\rR\x0bLKJHCMMPBDLb\x06proto3\
";

/// `FileDescriptorProto` object which was a source for this generated file
fn file_descriptor_proto() -> &'static ::protobuf::descriptor::FileDescriptorProto {
    static file_descriptor_proto_lazy: ::protobuf::rt::Lazy<::protobuf::descriptor::FileDescriptorProto> = ::protobuf::rt::Lazy::new();
    file_descriptor_proto_lazy.get(|| {
        ::protobuf::Message::parse_from_bytes(file_descriptor_proto_data).unwrap()
    })
}

/// `FileDescriptor` object which allows dynamic access to files
pub fn file_descriptor() -> &'static ::protobuf::reflect::FileDescriptor {
    static generated_file_descriptor_lazy: ::protobuf::rt::Lazy<::protobuf::reflect::GeneratedFileDescriptor> = ::protobuf::rt::Lazy::new();
    static file_descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::FileDescriptor> = ::protobuf::rt::Lazy::new();
    file_descriptor.get(|| {
        let generated_file_descriptor = generated_file_descriptor_lazy.get(|| {
            let mut deps = ::std::vec::Vec::with_capacity(1);
            deps.push(super::DDNJKAMIHHI::file_descriptor().clone());
            let mut messages = ::std::vec::Vec::with_capacity(1);
            messages.push(OENDLKLJHPF::generated_message_descriptor_data());
            let mut enums = ::std::vec::Vec::with_capacity(0);
            ::protobuf::reflect::GeneratedFileDescriptor::new_generated(
                file_descriptor_proto(),
                deps,
                messages,
                enums,
            )
        });
        ::protobuf::reflect::FileDescriptor::new_generated_2(generated_file_descriptor)
    })
}
