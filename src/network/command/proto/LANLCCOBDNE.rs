// This file is generated by rust-protobuf 3.7.1. Do not edit
// .proto file is parsed by pure
// @generated

// https://github.com/rust-lang/rust-clippy/issues/702
#![allow(unknown_lints)]
#![allow(clippy::all)]

#![allow(unused_attributes)]
#![cfg_attr(rustfmt, rustfmt::skip)]

#![allow(dead_code)]
#![allow(missing_docs)]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(trivial_casts)]
#![allow(unused_results)]
#![allow(unused_mut)]

//! Generated file from `LANLCCOBDNE.proto`

/// Generated files are compatible only with the same version
/// of protobuf runtime.
const _PROTOBUF_VERSION_CHECK: () = ::protobuf::VERSION_3_7_1;

// @@protoc_insertion_point(message:LANLCCOBDNE)
#[derive(PartialEq,Clone,Default,Debug)]
pub struct LANLCCOBDNE {
    // message fields
    // @@protoc_insertion_point(field:LANLCCOBDNE.PHOEJJDPDIL)
    pub PHOEJJDPDIL: ::std::vec::Vec<u32>,
    // @@protoc_insertion_point(field:LANLCCOBDNE.event_unique_id)
    pub event_unique_id: u32,
    // @@protoc_insertion_point(field:LANLCCOBDNE.dialogue_event_param_list)
    pub dialogue_event_param_list: ::std::vec::Vec<super::LLOCDPPMJNN::LLOCDPPMJNN>,
    // @@protoc_insertion_point(field:LANLCCOBDNE.talk_dialogue_id)
    pub talk_dialogue_id: u32,
    // @@protoc_insertion_point(field:LANLCCOBDNE.game_mode_type)
    pub game_mode_type: u32,
    // @@protoc_insertion_point(field:LANLCCOBDNE.aeon_talk_id)
    pub aeon_talk_id: u32,
    // special fields
    // @@protoc_insertion_point(special_field:LANLCCOBDNE.special_fields)
    pub special_fields: ::protobuf::SpecialFields,
}

impl<'a> ::std::default::Default for &'a LANLCCOBDNE {
    fn default() -> &'a LANLCCOBDNE {
        <LANLCCOBDNE as ::protobuf::Message>::default_instance()
    }
}

impl LANLCCOBDNE {
    pub fn new() -> LANLCCOBDNE {
        ::std::default::Default::default()
    }

    fn generated_message_descriptor_data() -> ::protobuf::reflect::GeneratedMessageDescriptorData {
        let mut fields = ::std::vec::Vec::with_capacity(6);
        let mut oneofs = ::std::vec::Vec::with_capacity(0);
        fields.push(::protobuf::reflect::rt::v2::make_vec_simpler_accessor::<_, _>(
            "PHOEJJDPDIL",
            |m: &LANLCCOBDNE| { &m.PHOEJJDPDIL },
            |m: &mut LANLCCOBDNE| { &mut m.PHOEJJDPDIL },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "event_unique_id",
            |m: &LANLCCOBDNE| { &m.event_unique_id },
            |m: &mut LANLCCOBDNE| { &mut m.event_unique_id },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_vec_simpler_accessor::<_, _>(
            "dialogue_event_param_list",
            |m: &LANLCCOBDNE| { &m.dialogue_event_param_list },
            |m: &mut LANLCCOBDNE| { &mut m.dialogue_event_param_list },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "talk_dialogue_id",
            |m: &LANLCCOBDNE| { &m.talk_dialogue_id },
            |m: &mut LANLCCOBDNE| { &mut m.talk_dialogue_id },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "game_mode_type",
            |m: &LANLCCOBDNE| { &m.game_mode_type },
            |m: &mut LANLCCOBDNE| { &mut m.game_mode_type },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "aeon_talk_id",
            |m: &LANLCCOBDNE| { &m.aeon_talk_id },
            |m: &mut LANLCCOBDNE| { &mut m.aeon_talk_id },
        ));
        ::protobuf::reflect::GeneratedMessageDescriptorData::new_2::<LANLCCOBDNE>(
            "LANLCCOBDNE",
            fields,
            oneofs,
        )
    }
}

impl ::protobuf::Message for LANLCCOBDNE {
    const NAME: &'static str = "LANLCCOBDNE";

    fn is_initialized(&self) -> bool {
        true
    }

    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::Result<()> {
        while let Some(tag) = is.read_raw_tag_or_eof()? {
            match tag {
                10 => {
                    is.read_repeated_packed_uint32_into(&mut self.PHOEJJDPDIL)?;
                },
                8 => {
                    self.PHOEJJDPDIL.push(is.read_uint32()?);
                },
                16 => {
                    self.event_unique_id = is.read_uint32()?;
                },
                74 => {
                    self.dialogue_event_param_list.push(is.read_message()?);
                },
                88 => {
                    self.talk_dialogue_id = is.read_uint32()?;
                },
                112 => {
                    self.game_mode_type = is.read_uint32()?;
                },
                120 => {
                    self.aeon_talk_id = is.read_uint32()?;
                },
                tag => {
                    ::protobuf::rt::read_unknown_or_skip_group(tag, is, self.special_fields.mut_unknown_fields())?;
                },
            };
        }
        ::std::result::Result::Ok(())
    }

    // Compute sizes of nested messages
    #[allow(unused_variables)]
    fn compute_size(&self) -> u64 {
        let mut my_size = 0;
        my_size += ::protobuf::rt::vec_packed_uint32_size(1, &self.PHOEJJDPDIL);
        if self.event_unique_id != 0 {
            my_size += ::protobuf::rt::uint32_size(2, self.event_unique_id);
        }
        for value in &self.dialogue_event_param_list {
            let len = value.compute_size();
            my_size += 1 + ::protobuf::rt::compute_raw_varint64_size(len) + len;
        };
        if self.talk_dialogue_id != 0 {
            my_size += ::protobuf::rt::uint32_size(11, self.talk_dialogue_id);
        }
        if self.game_mode_type != 0 {
            my_size += ::protobuf::rt::uint32_size(14, self.game_mode_type);
        }
        if self.aeon_talk_id != 0 {
            my_size += ::protobuf::rt::uint32_size(15, self.aeon_talk_id);
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.special_fields.unknown_fields());
        self.special_fields.cached_size().set(my_size as u32);
        my_size
    }

    fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::Result<()> {
        os.write_repeated_packed_uint32(1, &self.PHOEJJDPDIL)?;
        if self.event_unique_id != 0 {
            os.write_uint32(2, self.event_unique_id)?;
        }
        for v in &self.dialogue_event_param_list {
            ::protobuf::rt::write_message_field_with_cached_size(9, v, os)?;
        };
        if self.talk_dialogue_id != 0 {
            os.write_uint32(11, self.talk_dialogue_id)?;
        }
        if self.game_mode_type != 0 {
            os.write_uint32(14, self.game_mode_type)?;
        }
        if self.aeon_talk_id != 0 {
            os.write_uint32(15, self.aeon_talk_id)?;
        }
        os.write_unknown_fields(self.special_fields.unknown_fields())?;
        ::std::result::Result::Ok(())
    }

    fn special_fields(&self) -> &::protobuf::SpecialFields {
        &self.special_fields
    }

    fn mut_special_fields(&mut self) -> &mut ::protobuf::SpecialFields {
        &mut self.special_fields
    }

    fn new() -> LANLCCOBDNE {
        LANLCCOBDNE::new()
    }

    fn clear(&mut self) {
        self.PHOEJJDPDIL.clear();
        self.event_unique_id = 0;
        self.dialogue_event_param_list.clear();
        self.talk_dialogue_id = 0;
        self.game_mode_type = 0;
        self.aeon_talk_id = 0;
        self.special_fields.clear();
    }

    fn default_instance() -> &'static LANLCCOBDNE {
        static instance: LANLCCOBDNE = LANLCCOBDNE {
            PHOEJJDPDIL: ::std::vec::Vec::new(),
            event_unique_id: 0,
            dialogue_event_param_list: ::std::vec::Vec::new(),
            talk_dialogue_id: 0,
            game_mode_type: 0,
            aeon_talk_id: 0,
            special_fields: ::protobuf::SpecialFields::new(),
        };
        &instance
    }
}

impl ::protobuf::MessageFull for LANLCCOBDNE {
    fn descriptor() -> ::protobuf::reflect::MessageDescriptor {
        static descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::MessageDescriptor> = ::protobuf::rt::Lazy::new();
        descriptor.get(|| file_descriptor().message_by_package_relative_name("LANLCCOBDNE").unwrap()).clone()
    }
}

impl ::std::fmt::Display for LANLCCOBDNE {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::protobuf::text_format::fmt(self, f)
    }
}

impl ::protobuf::reflect::ProtobufValue for LANLCCOBDNE {
    type RuntimeType = ::protobuf::reflect::rt::RuntimeTypeMessage<Self>;
}

static file_descriptor_proto_data: &'static [u8] = b"\
    \n\x11LANLCCOBDNE.proto\x1a\x11LLOCDPPMJNN.proto\"\x92\x02\n\x0bLANLCCOB\
    DNE\x12\x20\n\x0bPHOEJJDPDIL\x18\x01\x20\x03(\rR\x0bPHOEJJDPDIL\x12&\n\
    \x0fevent_unique_id\x18\x02\x20\x01(\rR\reventUniqueId\x12G\n\x19dialogu\
    e_event_param_list\x18\t\x20\x03(\x0b2\x0c.LLOCDPPMJNNR\x16dialogueEvent\
    ParamList\x12(\n\x10talk_dialogue_id\x18\x0b\x20\x01(\rR\x0etalkDialogue\
    Id\x12$\n\x0egame_mode_type\x18\x0e\x20\x01(\rR\x0cgameModeType\x12\x20\
    \n\x0caeon_talk_id\x18\x0f\x20\x01(\rR\naeonTalkIdb\x06proto3\
";

/// `FileDescriptorProto` object which was a source for this generated file
fn file_descriptor_proto() -> &'static ::protobuf::descriptor::FileDescriptorProto {
    static file_descriptor_proto_lazy: ::protobuf::rt::Lazy<::protobuf::descriptor::FileDescriptorProto> = ::protobuf::rt::Lazy::new();
    file_descriptor_proto_lazy.get(|| {
        ::protobuf::Message::parse_from_bytes(file_descriptor_proto_data).unwrap()
    })
}

/// `FileDescriptor` object which allows dynamic access to files
pub fn file_descriptor() -> &'static ::protobuf::reflect::FileDescriptor {
    static generated_file_descriptor_lazy: ::protobuf::rt::Lazy<::protobuf::reflect::GeneratedFileDescriptor> = ::protobuf::rt::Lazy::new();
    static file_descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::FileDescriptor> = ::protobuf::rt::Lazy::new();
    file_descriptor.get(|| {
        let generated_file_descriptor = generated_file_descriptor_lazy.get(|| {
            let mut deps = ::std::vec::Vec::with_capacity(1);
            deps.push(super::LLOCDPPMJNN::file_descriptor().clone());
            let mut messages = ::std::vec::Vec::with_capacity(1);
            messages.push(LANLCCOBDNE::generated_message_descriptor_data());
            let mut enums = ::std::vec::Vec::with_capacity(0);
            ::protobuf::reflect::GeneratedFileDescriptor::new_generated(
                file_descriptor_proto(),
                deps,
                messages,
                enums,
            )
        });
        ::protobuf::reflect::FileDescriptor::new_generated_2(generated_file_descriptor)
    })
}
