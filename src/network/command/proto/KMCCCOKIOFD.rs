// This file is generated by rust-protobuf 3.7.1. Do not edit
// .proto file is parsed by pure
// @generated

// https://github.com/rust-lang/rust-clippy/issues/702
#![allow(unknown_lints)]
#![allow(clippy::all)]

#![allow(unused_attributes)]
#![cfg_attr(rustfmt, rustfmt::skip)]

#![allow(dead_code)]
#![allow(missing_docs)]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(trivial_casts)]
#![allow(unused_results)]
#![allow(unused_mut)]

//! Generated file from `KMCCCOKIOFD.proto`

/// Generated files are compatible only with the same version
/// of protobuf runtime.
const _PROTOBUF_VERSION_CHECK: () = ::protobuf::VERSION_3_7_1;

// @@protoc_insertion_point(message:KMCCCOKIOFD)
#[derive(PartialEq,Clone,Default,Debug)]
pub struct KMCCCOKIOFD {
    // message fields
    // @@protoc_insertion_point(field:KMCCCOKIOFD.GDNEHLEOMOM)
    pub GDNEHLEOMOM: u32,
    // @@protoc_insertion_point(field:KMCCCOKIOFD.FBNBPPHOGLI)
    pub FBNBPPHOGLI: u32,
    // @@protoc_insertion_point(field:KMCCCOKIOFD.buff_id)
    pub buff_id: u32,
    // @@protoc_insertion_point(field:KMCCCOKIOFD.EGKLFMCHLCO)
    pub EGKLFMCHLCO: u32,
    // @@protoc_insertion_point(field:KMCCCOKIOFD.JBGENFIKGGF)
    pub JBGENFIKGGF: u32,
    // special fields
    // @@protoc_insertion_point(special_field:KMCCCOKIOFD.special_fields)
    pub special_fields: ::protobuf::SpecialFields,
}

impl<'a> ::std::default::Default for &'a KMCCCOKIOFD {
    fn default() -> &'a KMCCCOKIOFD {
        <KMCCCOKIOFD as ::protobuf::Message>::default_instance()
    }
}

impl KMCCCOKIOFD {
    pub fn new() -> KMCCCOKIOFD {
        ::std::default::Default::default()
    }

    fn generated_message_descriptor_data() -> ::protobuf::reflect::GeneratedMessageDescriptorData {
        let mut fields = ::std::vec::Vec::with_capacity(5);
        let mut oneofs = ::std::vec::Vec::with_capacity(0);
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "GDNEHLEOMOM",
            |m: &KMCCCOKIOFD| { &m.GDNEHLEOMOM },
            |m: &mut KMCCCOKIOFD| { &mut m.GDNEHLEOMOM },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "FBNBPPHOGLI",
            |m: &KMCCCOKIOFD| { &m.FBNBPPHOGLI },
            |m: &mut KMCCCOKIOFD| { &mut m.FBNBPPHOGLI },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "buff_id",
            |m: &KMCCCOKIOFD| { &m.buff_id },
            |m: &mut KMCCCOKIOFD| { &mut m.buff_id },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "EGKLFMCHLCO",
            |m: &KMCCCOKIOFD| { &m.EGKLFMCHLCO },
            |m: &mut KMCCCOKIOFD| { &mut m.EGKLFMCHLCO },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "JBGENFIKGGF",
            |m: &KMCCCOKIOFD| { &m.JBGENFIKGGF },
            |m: &mut KMCCCOKIOFD| { &mut m.JBGENFIKGGF },
        ));
        ::protobuf::reflect::GeneratedMessageDescriptorData::new_2::<KMCCCOKIOFD>(
            "KMCCCOKIOFD",
            fields,
            oneofs,
        )
    }
}

impl ::protobuf::Message for KMCCCOKIOFD {
    const NAME: &'static str = "KMCCCOKIOFD";

    fn is_initialized(&self) -> bool {
        true
    }

    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::Result<()> {
        while let Some(tag) = is.read_raw_tag_or_eof()? {
            match tag {
                40 => {
                    self.GDNEHLEOMOM = is.read_uint32()?;
                },
                88 => {
                    self.FBNBPPHOGLI = is.read_uint32()?;
                },
                72 => {
                    self.buff_id = is.read_uint32()?;
                },
                8 => {
                    self.EGKLFMCHLCO = is.read_uint32()?;
                },
                112 => {
                    self.JBGENFIKGGF = is.read_uint32()?;
                },
                tag => {
                    ::protobuf::rt::read_unknown_or_skip_group(tag, is, self.special_fields.mut_unknown_fields())?;
                },
            };
        }
        ::std::result::Result::Ok(())
    }

    // Compute sizes of nested messages
    #[allow(unused_variables)]
    fn compute_size(&self) -> u64 {
        let mut my_size = 0;
        if self.GDNEHLEOMOM != 0 {
            my_size += ::protobuf::rt::uint32_size(5, self.GDNEHLEOMOM);
        }
        if self.FBNBPPHOGLI != 0 {
            my_size += ::protobuf::rt::uint32_size(11, self.FBNBPPHOGLI);
        }
        if self.buff_id != 0 {
            my_size += ::protobuf::rt::uint32_size(9, self.buff_id);
        }
        if self.EGKLFMCHLCO != 0 {
            my_size += ::protobuf::rt::uint32_size(1, self.EGKLFMCHLCO);
        }
        if self.JBGENFIKGGF != 0 {
            my_size += ::protobuf::rt::uint32_size(14, self.JBGENFIKGGF);
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.special_fields.unknown_fields());
        self.special_fields.cached_size().set(my_size as u32);
        my_size
    }

    fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::Result<()> {
        if self.GDNEHLEOMOM != 0 {
            os.write_uint32(5, self.GDNEHLEOMOM)?;
        }
        if self.FBNBPPHOGLI != 0 {
            os.write_uint32(11, self.FBNBPPHOGLI)?;
        }
        if self.buff_id != 0 {
            os.write_uint32(9, self.buff_id)?;
        }
        if self.EGKLFMCHLCO != 0 {
            os.write_uint32(1, self.EGKLFMCHLCO)?;
        }
        if self.JBGENFIKGGF != 0 {
            os.write_uint32(14, self.JBGENFIKGGF)?;
        }
        os.write_unknown_fields(self.special_fields.unknown_fields())?;
        ::std::result::Result::Ok(())
    }

    fn special_fields(&self) -> &::protobuf::SpecialFields {
        &self.special_fields
    }

    fn mut_special_fields(&mut self) -> &mut ::protobuf::SpecialFields {
        &mut self.special_fields
    }

    fn new() -> KMCCCOKIOFD {
        KMCCCOKIOFD::new()
    }

    fn clear(&mut self) {
        self.GDNEHLEOMOM = 0;
        self.FBNBPPHOGLI = 0;
        self.buff_id = 0;
        self.EGKLFMCHLCO = 0;
        self.JBGENFIKGGF = 0;
        self.special_fields.clear();
    }

    fn default_instance() -> &'static KMCCCOKIOFD {
        static instance: KMCCCOKIOFD = KMCCCOKIOFD {
            GDNEHLEOMOM: 0,
            FBNBPPHOGLI: 0,
            buff_id: 0,
            EGKLFMCHLCO: 0,
            JBGENFIKGGF: 0,
            special_fields: ::protobuf::SpecialFields::new(),
        };
        &instance
    }
}

impl ::protobuf::MessageFull for KMCCCOKIOFD {
    fn descriptor() -> ::protobuf::reflect::MessageDescriptor {
        static descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::MessageDescriptor> = ::protobuf::rt::Lazy::new();
        descriptor.get(|| file_descriptor().message_by_package_relative_name("KMCCCOKIOFD").unwrap()).clone()
    }
}

impl ::std::fmt::Display for KMCCCOKIOFD {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::protobuf::text_format::fmt(self, f)
    }
}

impl ::protobuf::reflect::ProtobufValue for KMCCCOKIOFD {
    type RuntimeType = ::protobuf::reflect::rt::RuntimeTypeMessage<Self>;
}

static file_descriptor_proto_data: &'static [u8] = b"\
    \n\x11KMCCCOKIOFD.proto\"\xae\x01\n\x0bKMCCCOKIOFD\x12\x20\n\x0bGDNEHLEO\
    MOM\x18\x05\x20\x01(\rR\x0bGDNEHLEOMOM\x12\x20\n\x0bFBNBPPHOGLI\x18\x0b\
    \x20\x01(\rR\x0bFBNBPPHOGLI\x12\x17\n\x07buff_id\x18\t\x20\x01(\rR\x06bu\
    ffId\x12\x20\n\x0bEGKLFMCHLCO\x18\x01\x20\x01(\rR\x0bEGKLFMCHLCO\x12\x20\
    \n\x0bJBGENFIKGGF\x18\x0e\x20\x01(\rR\x0bJBGENFIKGGFb\x06proto3\
";

/// `FileDescriptorProto` object which was a source for this generated file
fn file_descriptor_proto() -> &'static ::protobuf::descriptor::FileDescriptorProto {
    static file_descriptor_proto_lazy: ::protobuf::rt::Lazy<::protobuf::descriptor::FileDescriptorProto> = ::protobuf::rt::Lazy::new();
    file_descriptor_proto_lazy.get(|| {
        ::protobuf::Message::parse_from_bytes(file_descriptor_proto_data).unwrap()
    })
}

/// `FileDescriptor` object which allows dynamic access to files
pub fn file_descriptor() -> &'static ::protobuf::reflect::FileDescriptor {
    static generated_file_descriptor_lazy: ::protobuf::rt::Lazy<::protobuf::reflect::GeneratedFileDescriptor> = ::protobuf::rt::Lazy::new();
    static file_descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::FileDescriptor> = ::protobuf::rt::Lazy::new();
    file_descriptor.get(|| {
        let generated_file_descriptor = generated_file_descriptor_lazy.get(|| {
            let mut deps = ::std::vec::Vec::with_capacity(0);
            let mut messages = ::std::vec::Vec::with_capacity(1);
            messages.push(KMCCCOKIOFD::generated_message_descriptor_data());
            let mut enums = ::std::vec::Vec::with_capacity(0);
            ::protobuf::reflect::GeneratedFileDescriptor::new_generated(
                file_descriptor_proto(),
                deps,
                messages,
                enums,
            )
        });
        ::protobuf::reflect::FileDescriptor::new_generated_2(generated_file_descriptor)
    })
}
