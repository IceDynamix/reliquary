// This file is generated by rust-protobuf 3.7.1. Do not edit
// .proto file is parsed by pure
// @generated

// https://github.com/rust-lang/rust-clippy/issues/702
#![allow(unknown_lints)]
#![allow(clippy::all)]

#![allow(unused_attributes)]
#![cfg_attr(rustfmt, rustfmt::skip)]

#![allow(dead_code)]
#![allow(missing_docs)]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(trivial_casts)]
#![allow(unused_results)]
#![allow(unused_mut)]

//! Generated file from `RogueMiracleSelectInfo.proto`

/// Generated files are compatible only with the same version
/// of protobuf runtime.
const _PROTOBUF_VERSION_CHECK: () = ::protobuf::VERSION_3_7_1;

// @@protoc_insertion_point(message:RogueMiracleSelectInfo)
#[derive(PartialEq,Clone,Default,Debug)]
pub struct RogueMiracleSelectInfo {
    // message fields
    // @@protoc_insertion_point(field:RogueMiracleSelectInfo.select_hint_id)
    pub select_hint_id: u32,
    // @@protoc_insertion_point(field:RogueMiracleSelectInfo.select_miracle_list)
    pub select_miracle_list: ::std::vec::Vec<u32>,
    // @@protoc_insertion_point(field:RogueMiracleSelectInfo.APIKLEGGDHM)
    pub APIKLEGGDHM: bool,
    // @@protoc_insertion_point(field:RogueMiracleSelectInfo.miracle_handbook_list)
    pub miracle_handbook_list: ::std::vec::Vec<u32>,
    // @@protoc_insertion_point(field:RogueMiracleSelectInfo.BMFCBCMCLAF)
    pub BMFCBCMCLAF: u32,
    // @@protoc_insertion_point(field:RogueMiracleSelectInfo.OOOECPAACCK)
    pub OOOECPAACCK: u32,
    // special fields
    // @@protoc_insertion_point(special_field:RogueMiracleSelectInfo.special_fields)
    pub special_fields: ::protobuf::SpecialFields,
}

impl<'a> ::std::default::Default for &'a RogueMiracleSelectInfo {
    fn default() -> &'a RogueMiracleSelectInfo {
        <RogueMiracleSelectInfo as ::protobuf::Message>::default_instance()
    }
}

impl RogueMiracleSelectInfo {
    pub fn new() -> RogueMiracleSelectInfo {
        ::std::default::Default::default()
    }

    fn generated_message_descriptor_data() -> ::protobuf::reflect::GeneratedMessageDescriptorData {
        let mut fields = ::std::vec::Vec::with_capacity(6);
        let mut oneofs = ::std::vec::Vec::with_capacity(0);
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "select_hint_id",
            |m: &RogueMiracleSelectInfo| { &m.select_hint_id },
            |m: &mut RogueMiracleSelectInfo| { &mut m.select_hint_id },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_vec_simpler_accessor::<_, _>(
            "select_miracle_list",
            |m: &RogueMiracleSelectInfo| { &m.select_miracle_list },
            |m: &mut RogueMiracleSelectInfo| { &mut m.select_miracle_list },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "APIKLEGGDHM",
            |m: &RogueMiracleSelectInfo| { &m.APIKLEGGDHM },
            |m: &mut RogueMiracleSelectInfo| { &mut m.APIKLEGGDHM },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_vec_simpler_accessor::<_, _>(
            "miracle_handbook_list",
            |m: &RogueMiracleSelectInfo| { &m.miracle_handbook_list },
            |m: &mut RogueMiracleSelectInfo| { &mut m.miracle_handbook_list },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "BMFCBCMCLAF",
            |m: &RogueMiracleSelectInfo| { &m.BMFCBCMCLAF },
            |m: &mut RogueMiracleSelectInfo| { &mut m.BMFCBCMCLAF },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "OOOECPAACCK",
            |m: &RogueMiracleSelectInfo| { &m.OOOECPAACCK },
            |m: &mut RogueMiracleSelectInfo| { &mut m.OOOECPAACCK },
        ));
        ::protobuf::reflect::GeneratedMessageDescriptorData::new_2::<RogueMiracleSelectInfo>(
            "RogueMiracleSelectInfo",
            fields,
            oneofs,
        )
    }
}

impl ::protobuf::Message for RogueMiracleSelectInfo {
    const NAME: &'static str = "RogueMiracleSelectInfo";

    fn is_initialized(&self) -> bool {
        true
    }

    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::Result<()> {
        while let Some(tag) = is.read_raw_tag_or_eof()? {
            match tag {
                56 => {
                    self.select_hint_id = is.read_uint32()?;
                },
                98 => {
                    is.read_repeated_packed_uint32_into(&mut self.select_miracle_list)?;
                },
                96 => {
                    self.select_miracle_list.push(is.read_uint32()?);
                },
                72 => {
                    self.APIKLEGGDHM = is.read_bool()?;
                },
                82 => {
                    is.read_repeated_packed_uint32_into(&mut self.miracle_handbook_list)?;
                },
                80 => {
                    self.miracle_handbook_list.push(is.read_uint32()?);
                },
                120 => {
                    self.BMFCBCMCLAF = is.read_uint32()?;
                },
                32 => {
                    self.OOOECPAACCK = is.read_uint32()?;
                },
                tag => {
                    ::protobuf::rt::read_unknown_or_skip_group(tag, is, self.special_fields.mut_unknown_fields())?;
                },
            };
        }
        ::std::result::Result::Ok(())
    }

    // Compute sizes of nested messages
    #[allow(unused_variables)]
    fn compute_size(&self) -> u64 {
        let mut my_size = 0;
        if self.select_hint_id != 0 {
            my_size += ::protobuf::rt::uint32_size(7, self.select_hint_id);
        }
        my_size += ::protobuf::rt::vec_packed_uint32_size(12, &self.select_miracle_list);
        if self.APIKLEGGDHM != false {
            my_size += 1 + 1;
        }
        my_size += ::protobuf::rt::vec_packed_uint32_size(10, &self.miracle_handbook_list);
        if self.BMFCBCMCLAF != 0 {
            my_size += ::protobuf::rt::uint32_size(15, self.BMFCBCMCLAF);
        }
        if self.OOOECPAACCK != 0 {
            my_size += ::protobuf::rt::uint32_size(4, self.OOOECPAACCK);
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.special_fields.unknown_fields());
        self.special_fields.cached_size().set(my_size as u32);
        my_size
    }

    fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::Result<()> {
        if self.select_hint_id != 0 {
            os.write_uint32(7, self.select_hint_id)?;
        }
        os.write_repeated_packed_uint32(12, &self.select_miracle_list)?;
        if self.APIKLEGGDHM != false {
            os.write_bool(9, self.APIKLEGGDHM)?;
        }
        os.write_repeated_packed_uint32(10, &self.miracle_handbook_list)?;
        if self.BMFCBCMCLAF != 0 {
            os.write_uint32(15, self.BMFCBCMCLAF)?;
        }
        if self.OOOECPAACCK != 0 {
            os.write_uint32(4, self.OOOECPAACCK)?;
        }
        os.write_unknown_fields(self.special_fields.unknown_fields())?;
        ::std::result::Result::Ok(())
    }

    fn special_fields(&self) -> &::protobuf::SpecialFields {
        &self.special_fields
    }

    fn mut_special_fields(&mut self) -> &mut ::protobuf::SpecialFields {
        &mut self.special_fields
    }

    fn new() -> RogueMiracleSelectInfo {
        RogueMiracleSelectInfo::new()
    }

    fn clear(&mut self) {
        self.select_hint_id = 0;
        self.select_miracle_list.clear();
        self.APIKLEGGDHM = false;
        self.miracle_handbook_list.clear();
        self.BMFCBCMCLAF = 0;
        self.OOOECPAACCK = 0;
        self.special_fields.clear();
    }

    fn default_instance() -> &'static RogueMiracleSelectInfo {
        static instance: RogueMiracleSelectInfo = RogueMiracleSelectInfo {
            select_hint_id: 0,
            select_miracle_list: ::std::vec::Vec::new(),
            APIKLEGGDHM: false,
            miracle_handbook_list: ::std::vec::Vec::new(),
            BMFCBCMCLAF: 0,
            OOOECPAACCK: 0,
            special_fields: ::protobuf::SpecialFields::new(),
        };
        &instance
    }
}

impl ::protobuf::MessageFull for RogueMiracleSelectInfo {
    fn descriptor() -> ::protobuf::reflect::MessageDescriptor {
        static descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::MessageDescriptor> = ::protobuf::rt::Lazy::new();
        descriptor.get(|| file_descriptor().message_by_package_relative_name("RogueMiracleSelectInfo").unwrap()).clone()
    }
}

impl ::std::fmt::Display for RogueMiracleSelectInfo {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::protobuf::text_format::fmt(self, f)
    }
}

impl ::protobuf::reflect::ProtobufValue for RogueMiracleSelectInfo {
    type RuntimeType = ::protobuf::reflect::rt::RuntimeTypeMessage<Self>;
}

static file_descriptor_proto_data: &'static [u8] = b"\
    \n\x1cRogueMiracleSelectInfo.proto\"\x88\x02\n\x16RogueMiracleSelectInfo\
    \x12$\n\x0eselect_hint_id\x18\x07\x20\x01(\rR\x0cselectHintId\x12.\n\x13\
    select_miracle_list\x18\x0c\x20\x03(\rR\x11selectMiracleList\x12\x20\n\
    \x0bAPIKLEGGDHM\x18\t\x20\x01(\x08R\x0bAPIKLEGGDHM\x122\n\x15miracle_han\
    dbook_list\x18\n\x20\x03(\rR\x13miracleHandbookList\x12\x20\n\x0bBMFCBCM\
    CLAF\x18\x0f\x20\x01(\rR\x0bBMFCBCMCLAF\x12\x20\n\x0bOOOECPAACCK\x18\x04\
    \x20\x01(\rR\x0bOOOECPAACCKb\x06proto3\
";

/// `FileDescriptorProto` object which was a source for this generated file
fn file_descriptor_proto() -> &'static ::protobuf::descriptor::FileDescriptorProto {
    static file_descriptor_proto_lazy: ::protobuf::rt::Lazy<::protobuf::descriptor::FileDescriptorProto> = ::protobuf::rt::Lazy::new();
    file_descriptor_proto_lazy.get(|| {
        ::protobuf::Message::parse_from_bytes(file_descriptor_proto_data).unwrap()
    })
}

/// `FileDescriptor` object which allows dynamic access to files
pub fn file_descriptor() -> &'static ::protobuf::reflect::FileDescriptor {
    static generated_file_descriptor_lazy: ::protobuf::rt::Lazy<::protobuf::reflect::GeneratedFileDescriptor> = ::protobuf::rt::Lazy::new();
    static file_descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::FileDescriptor> = ::protobuf::rt::Lazy::new();
    file_descriptor.get(|| {
        let generated_file_descriptor = generated_file_descriptor_lazy.get(|| {
            let mut deps = ::std::vec::Vec::with_capacity(0);
            let mut messages = ::std::vec::Vec::with_capacity(1);
            messages.push(RogueMiracleSelectInfo::generated_message_descriptor_data());
            let mut enums = ::std::vec::Vec::with_capacity(0);
            ::protobuf::reflect::GeneratedFileDescriptor::new_generated(
                file_descriptor_proto(),
                deps,
                messages,
                enums,
            )
        });
        ::protobuf::reflect::FileDescriptor::new_generated_2(generated_file_descriptor)
    })
}
