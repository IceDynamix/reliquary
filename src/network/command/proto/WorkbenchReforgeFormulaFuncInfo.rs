// This file is generated by rust-protobuf 3.7.1. Do not edit
// .proto file is parsed by pure
// @generated

// https://github.com/rust-lang/rust-clippy/issues/702
#![allow(unknown_lints)]
#![allow(clippy::all)]

#![allow(unused_attributes)]
#![cfg_attr(rustfmt, rustfmt::skip)]

#![allow(dead_code)]
#![allow(missing_docs)]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(trivial_casts)]
#![allow(unused_results)]
#![allow(unused_mut)]

//! Generated file from `WorkbenchReforgeFormulaFuncInfo.proto`

/// Generated files are compatible only with the same version
/// of protobuf runtime.
const _PROTOBUF_VERSION_CHECK: () = ::protobuf::VERSION_3_7_1;

// @@protoc_insertion_point(message:WorkbenchReforgeFormulaFuncInfo)
#[derive(PartialEq,Clone,Default,Debug)]
pub struct WorkbenchReforgeFormulaFuncInfo {
    // message fields
    // @@protoc_insertion_point(field:WorkbenchReforgeFormulaFuncInfo.JGEHHOAGFEP)
    pub JGEHHOAGFEP: u32,
    // @@protoc_insertion_point(field:WorkbenchReforgeFormulaFuncInfo.cost_data)
    pub cost_data: ::protobuf::MessageField<super::ItemCostData::ItemCostData>,
    // @@protoc_insertion_point(field:WorkbenchReforgeFormulaFuncInfo.int_reforge_num_value)
    pub int_reforge_num_value: i32,
    // @@protoc_insertion_point(field:WorkbenchReforgeFormulaFuncInfo.can_free_reforge)
    pub can_free_reforge: bool,
    // @@protoc_insertion_point(field:WorkbenchReforgeFormulaFuncInfo.free_reforge_num)
    pub free_reforge_num: u32,
    // special fields
    // @@protoc_insertion_point(special_field:WorkbenchReforgeFormulaFuncInfo.special_fields)
    pub special_fields: ::protobuf::SpecialFields,
}

impl<'a> ::std::default::Default for &'a WorkbenchReforgeFormulaFuncInfo {
    fn default() -> &'a WorkbenchReforgeFormulaFuncInfo {
        <WorkbenchReforgeFormulaFuncInfo as ::protobuf::Message>::default_instance()
    }
}

impl WorkbenchReforgeFormulaFuncInfo {
    pub fn new() -> WorkbenchReforgeFormulaFuncInfo {
        ::std::default::Default::default()
    }

    fn generated_message_descriptor_data() -> ::protobuf::reflect::GeneratedMessageDescriptorData {
        let mut fields = ::std::vec::Vec::with_capacity(5);
        let mut oneofs = ::std::vec::Vec::with_capacity(0);
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "JGEHHOAGFEP",
            |m: &WorkbenchReforgeFormulaFuncInfo| { &m.JGEHHOAGFEP },
            |m: &mut WorkbenchReforgeFormulaFuncInfo| { &mut m.JGEHHOAGFEP },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_message_field_accessor::<_, super::ItemCostData::ItemCostData>(
            "cost_data",
            |m: &WorkbenchReforgeFormulaFuncInfo| { &m.cost_data },
            |m: &mut WorkbenchReforgeFormulaFuncInfo| { &mut m.cost_data },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "int_reforge_num_value",
            |m: &WorkbenchReforgeFormulaFuncInfo| { &m.int_reforge_num_value },
            |m: &mut WorkbenchReforgeFormulaFuncInfo| { &mut m.int_reforge_num_value },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "can_free_reforge",
            |m: &WorkbenchReforgeFormulaFuncInfo| { &m.can_free_reforge },
            |m: &mut WorkbenchReforgeFormulaFuncInfo| { &mut m.can_free_reforge },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "free_reforge_num",
            |m: &WorkbenchReforgeFormulaFuncInfo| { &m.free_reforge_num },
            |m: &mut WorkbenchReforgeFormulaFuncInfo| { &mut m.free_reforge_num },
        ));
        ::protobuf::reflect::GeneratedMessageDescriptorData::new_2::<WorkbenchReforgeFormulaFuncInfo>(
            "WorkbenchReforgeFormulaFuncInfo",
            fields,
            oneofs,
        )
    }
}

impl ::protobuf::Message for WorkbenchReforgeFormulaFuncInfo {
    const NAME: &'static str = "WorkbenchReforgeFormulaFuncInfo";

    fn is_initialized(&self) -> bool {
        true
    }

    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::Result<()> {
        while let Some(tag) = is.read_raw_tag_or_eof()? {
            match tag {
                120 => {
                    self.JGEHHOAGFEP = is.read_uint32()?;
                },
                34 => {
                    ::protobuf::rt::read_singular_message_into_field(is, &mut self.cost_data)?;
                },
                72 => {
                    self.int_reforge_num_value = is.read_int32()?;
                },
                16 => {
                    self.can_free_reforge = is.read_bool()?;
                },
                104 => {
                    self.free_reforge_num = is.read_uint32()?;
                },
                tag => {
                    ::protobuf::rt::read_unknown_or_skip_group(tag, is, self.special_fields.mut_unknown_fields())?;
                },
            };
        }
        ::std::result::Result::Ok(())
    }

    // Compute sizes of nested messages
    #[allow(unused_variables)]
    fn compute_size(&self) -> u64 {
        let mut my_size = 0;
        if self.JGEHHOAGFEP != 0 {
            my_size += ::protobuf::rt::uint32_size(15, self.JGEHHOAGFEP);
        }
        if let Some(v) = self.cost_data.as_ref() {
            let len = v.compute_size();
            my_size += 1 + ::protobuf::rt::compute_raw_varint64_size(len) + len;
        }
        if self.int_reforge_num_value != 0 {
            my_size += ::protobuf::rt::int32_size(9, self.int_reforge_num_value);
        }
        if self.can_free_reforge != false {
            my_size += 1 + 1;
        }
        if self.free_reforge_num != 0 {
            my_size += ::protobuf::rt::uint32_size(13, self.free_reforge_num);
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.special_fields.unknown_fields());
        self.special_fields.cached_size().set(my_size as u32);
        my_size
    }

    fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::Result<()> {
        if self.JGEHHOAGFEP != 0 {
            os.write_uint32(15, self.JGEHHOAGFEP)?;
        }
        if let Some(v) = self.cost_data.as_ref() {
            ::protobuf::rt::write_message_field_with_cached_size(4, v, os)?;
        }
        if self.int_reforge_num_value != 0 {
            os.write_int32(9, self.int_reforge_num_value)?;
        }
        if self.can_free_reforge != false {
            os.write_bool(2, self.can_free_reforge)?;
        }
        if self.free_reforge_num != 0 {
            os.write_uint32(13, self.free_reforge_num)?;
        }
        os.write_unknown_fields(self.special_fields.unknown_fields())?;
        ::std::result::Result::Ok(())
    }

    fn special_fields(&self) -> &::protobuf::SpecialFields {
        &self.special_fields
    }

    fn mut_special_fields(&mut self) -> &mut ::protobuf::SpecialFields {
        &mut self.special_fields
    }

    fn new() -> WorkbenchReforgeFormulaFuncInfo {
        WorkbenchReforgeFormulaFuncInfo::new()
    }

    fn clear(&mut self) {
        self.JGEHHOAGFEP = 0;
        self.cost_data.clear();
        self.int_reforge_num_value = 0;
        self.can_free_reforge = false;
        self.free_reforge_num = 0;
        self.special_fields.clear();
    }

    fn default_instance() -> &'static WorkbenchReforgeFormulaFuncInfo {
        static instance: WorkbenchReforgeFormulaFuncInfo = WorkbenchReforgeFormulaFuncInfo {
            JGEHHOAGFEP: 0,
            cost_data: ::protobuf::MessageField::none(),
            int_reforge_num_value: 0,
            can_free_reforge: false,
            free_reforge_num: 0,
            special_fields: ::protobuf::SpecialFields::new(),
        };
        &instance
    }
}

impl ::protobuf::MessageFull for WorkbenchReforgeFormulaFuncInfo {
    fn descriptor() -> ::protobuf::reflect::MessageDescriptor {
        static descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::MessageDescriptor> = ::protobuf::rt::Lazy::new();
        descriptor.get(|| file_descriptor().message_by_package_relative_name("WorkbenchReforgeFormulaFuncInfo").unwrap()).clone()
    }
}

impl ::std::fmt::Display for WorkbenchReforgeFormulaFuncInfo {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::protobuf::text_format::fmt(self, f)
    }
}

impl ::protobuf::reflect::ProtobufValue for WorkbenchReforgeFormulaFuncInfo {
    type RuntimeType = ::protobuf::reflect::rt::RuntimeTypeMessage<Self>;
}

static file_descriptor_proto_data: &'static [u8] = b"\
    \n%WorkbenchReforgeFormulaFuncInfo.proto\x1a\x12ItemCostData.proto\"\xf6\
    \x01\n\x1fWorkbenchReforgeFormulaFuncInfo\x12\x20\n\x0bJGEHHOAGFEP\x18\
    \x0f\x20\x01(\rR\x0bJGEHHOAGFEP\x12*\n\tcost_data\x18\x04\x20\x01(\x0b2\
    \r.ItemCostDataR\x08costData\x121\n\x15int_reforge_num_value\x18\t\x20\
    \x01(\x05R\x12intReforgeNumValue\x12(\n\x10can_free_reforge\x18\x02\x20\
    \x01(\x08R\x0ecanFreeReforge\x12(\n\x10free_reforge_num\x18\r\x20\x01(\r\
    R\x0efreeReforgeNumb\x06proto3\
";

/// `FileDescriptorProto` object which was a source for this generated file
fn file_descriptor_proto() -> &'static ::protobuf::descriptor::FileDescriptorProto {
    static file_descriptor_proto_lazy: ::protobuf::rt::Lazy<::protobuf::descriptor::FileDescriptorProto> = ::protobuf::rt::Lazy::new();
    file_descriptor_proto_lazy.get(|| {
        ::protobuf::Message::parse_from_bytes(file_descriptor_proto_data).unwrap()
    })
}

/// `FileDescriptor` object which allows dynamic access to files
pub fn file_descriptor() -> &'static ::protobuf::reflect::FileDescriptor {
    static generated_file_descriptor_lazy: ::protobuf::rt::Lazy<::protobuf::reflect::GeneratedFileDescriptor> = ::protobuf::rt::Lazy::new();
    static file_descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::FileDescriptor> = ::protobuf::rt::Lazy::new();
    file_descriptor.get(|| {
        let generated_file_descriptor = generated_file_descriptor_lazy.get(|| {
            let mut deps = ::std::vec::Vec::with_capacity(1);
            deps.push(super::ItemCostData::file_descriptor().clone());
            let mut messages = ::std::vec::Vec::with_capacity(1);
            messages.push(WorkbenchReforgeFormulaFuncInfo::generated_message_descriptor_data());
            let mut enums = ::std::vec::Vec::with_capacity(0);
            ::protobuf::reflect::GeneratedFileDescriptor::new_generated(
                file_descriptor_proto(),
                deps,
                messages,
                enums,
            )
        });
        ::protobuf::reflect::FileDescriptor::new_generated_2(generated_file_descriptor)
    })
}
