// This file is generated by rust-protobuf 3.7.1. Do not edit
// .proto file is parsed by pure
// @generated

// https://github.com/rust-lang/rust-clippy/issues/702
#![allow(unknown_lints)]
#![allow(clippy::all)]

#![allow(unused_attributes)]
#![cfg_attr(rustfmt, rustfmt::skip)]

#![allow(dead_code)]
#![allow(missing_docs)]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(trivial_casts)]
#![allow(unused_results)]
#![allow(unused_mut)]

//! Generated file from `FLCJMPNDMMH.proto`

/// Generated files are compatible only with the same version
/// of protobuf runtime.
const _PROTOBUF_VERSION_CHECK: () = ::protobuf::VERSION_3_7_1;

// @@protoc_insertion_point(message:FLCJMPNDMMH)
#[derive(PartialEq,Clone,Default,Debug)]
pub struct FLCJMPNDMMH {
    // message fields
    // @@protoc_insertion_point(field:FLCJMPNDMMH.HBMGKJAAPFI)
    pub HBMGKJAAPFI: u32,
    // @@protoc_insertion_point(field:FLCJMPNDMMH.BFEOOJBNFBG)
    pub BFEOOJBNFBG: ::protobuf::MessageField<super::NCFKHDIKCNI::NCFKHDIKCNI>,
    // @@protoc_insertion_point(field:FLCJMPNDMMH.MAGBFEHEOOI)
    pub MAGBFEHEOOI: u32,
    // @@protoc_insertion_point(field:FLCJMPNDMMH.FIHPPANOOFL)
    pub FIHPPANOOFL: ::std::vec::Vec<super::GLECNCOBKCJ::GLECNCOBKCJ>,
    // @@protoc_insertion_point(field:FLCJMPNDMMH.NFCMAEIGKHK)
    pub NFCMAEIGKHK: u32,
    // @@protoc_insertion_point(field:FLCJMPNDMMH.BPBADPAACOL)
    pub BPBADPAACOL: u32,
    // @@protoc_insertion_point(field:FLCJMPNDMMH.LAAAPJAIKAF)
    pub LAAAPJAIKAF: u32,
    // @@protoc_insertion_point(field:FLCJMPNDMMH.FDGKBOIEHBD)
    pub FDGKBOIEHBD: ::protobuf::MessageField<super::NCFKHDIKCNI::NCFKHDIKCNI>,
    // @@protoc_insertion_point(field:FLCJMPNDMMH.ODICAOCLJKO)
    pub ODICAOCLJKO: ::std::collections::HashMap<u32, u32>,
    // @@protoc_insertion_point(field:FLCJMPNDMMH.FHGFEKEAJFF)
    pub FHGFEKEAJFF: ::protobuf::MessageField<super::NCFKHDIKCNI::NCFKHDIKCNI>,
    // special fields
    // @@protoc_insertion_point(special_field:FLCJMPNDMMH.special_fields)
    pub special_fields: ::protobuf::SpecialFields,
}

impl<'a> ::std::default::Default for &'a FLCJMPNDMMH {
    fn default() -> &'a FLCJMPNDMMH {
        <FLCJMPNDMMH as ::protobuf::Message>::default_instance()
    }
}

impl FLCJMPNDMMH {
    pub fn new() -> FLCJMPNDMMH {
        ::std::default::Default::default()
    }

    fn generated_message_descriptor_data() -> ::protobuf::reflect::GeneratedMessageDescriptorData {
        let mut fields = ::std::vec::Vec::with_capacity(10);
        let mut oneofs = ::std::vec::Vec::with_capacity(0);
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "HBMGKJAAPFI",
            |m: &FLCJMPNDMMH| { &m.HBMGKJAAPFI },
            |m: &mut FLCJMPNDMMH| { &mut m.HBMGKJAAPFI },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_message_field_accessor::<_, super::NCFKHDIKCNI::NCFKHDIKCNI>(
            "BFEOOJBNFBG",
            |m: &FLCJMPNDMMH| { &m.BFEOOJBNFBG },
            |m: &mut FLCJMPNDMMH| { &mut m.BFEOOJBNFBG },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "MAGBFEHEOOI",
            |m: &FLCJMPNDMMH| { &m.MAGBFEHEOOI },
            |m: &mut FLCJMPNDMMH| { &mut m.MAGBFEHEOOI },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_vec_simpler_accessor::<_, _>(
            "FIHPPANOOFL",
            |m: &FLCJMPNDMMH| { &m.FIHPPANOOFL },
            |m: &mut FLCJMPNDMMH| { &mut m.FIHPPANOOFL },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "NFCMAEIGKHK",
            |m: &FLCJMPNDMMH| { &m.NFCMAEIGKHK },
            |m: &mut FLCJMPNDMMH| { &mut m.NFCMAEIGKHK },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "BPBADPAACOL",
            |m: &FLCJMPNDMMH| { &m.BPBADPAACOL },
            |m: &mut FLCJMPNDMMH| { &mut m.BPBADPAACOL },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_simpler_field_accessor::<_, _>(
            "LAAAPJAIKAF",
            |m: &FLCJMPNDMMH| { &m.LAAAPJAIKAF },
            |m: &mut FLCJMPNDMMH| { &mut m.LAAAPJAIKAF },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_message_field_accessor::<_, super::NCFKHDIKCNI::NCFKHDIKCNI>(
            "FDGKBOIEHBD",
            |m: &FLCJMPNDMMH| { &m.FDGKBOIEHBD },
            |m: &mut FLCJMPNDMMH| { &mut m.FDGKBOIEHBD },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_map_simpler_accessor_new::<_, _>(
            "ODICAOCLJKO",
            |m: &FLCJMPNDMMH| { &m.ODICAOCLJKO },
            |m: &mut FLCJMPNDMMH| { &mut m.ODICAOCLJKO },
        ));
        fields.push(::protobuf::reflect::rt::v2::make_message_field_accessor::<_, super::NCFKHDIKCNI::NCFKHDIKCNI>(
            "FHGFEKEAJFF",
            |m: &FLCJMPNDMMH| { &m.FHGFEKEAJFF },
            |m: &mut FLCJMPNDMMH| { &mut m.FHGFEKEAJFF },
        ));
        ::protobuf::reflect::GeneratedMessageDescriptorData::new_2::<FLCJMPNDMMH>(
            "FLCJMPNDMMH",
            fields,
            oneofs,
        )
    }
}

impl ::protobuf::Message for FLCJMPNDMMH {
    const NAME: &'static str = "FLCJMPNDMMH";

    fn is_initialized(&self) -> bool {
        true
    }

    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::Result<()> {
        while let Some(tag) = is.read_raw_tag_or_eof()? {
            match tag {
                16 => {
                    self.HBMGKJAAPFI = is.read_uint32()?;
                },
                26 => {
                    ::protobuf::rt::read_singular_message_into_field(is, &mut self.BFEOOJBNFBG)?;
                },
                32 => {
                    self.MAGBFEHEOOI = is.read_uint32()?;
                },
                50 => {
                    self.FIHPPANOOFL.push(is.read_message()?);
                },
                56 => {
                    self.NFCMAEIGKHK = is.read_uint32()?;
                },
                80 => {
                    self.BPBADPAACOL = is.read_uint32()?;
                },
                96 => {
                    self.LAAAPJAIKAF = is.read_uint32()?;
                },
                106 => {
                    ::protobuf::rt::read_singular_message_into_field(is, &mut self.FDGKBOIEHBD)?;
                },
                114 => {
                    let len = is.read_raw_varint32()?;
                    let old_limit = is.push_limit(len as u64)?;
                    let mut key = ::std::default::Default::default();
                    let mut value = ::std::default::Default::default();
                    while let Some(tag) = is.read_raw_tag_or_eof()? {
                        match tag {
                            8 => key = is.read_uint32()?,
                            16 => value = is.read_uint32()?,
                            _ => ::protobuf::rt::skip_field_for_tag(tag, is)?,
                        };
                    }
                    is.pop_limit(old_limit);
                    self.ODICAOCLJKO.insert(key, value);
                },
                122 => {
                    ::protobuf::rt::read_singular_message_into_field(is, &mut self.FHGFEKEAJFF)?;
                },
                tag => {
                    ::protobuf::rt::read_unknown_or_skip_group(tag, is, self.special_fields.mut_unknown_fields())?;
                },
            };
        }
        ::std::result::Result::Ok(())
    }

    // Compute sizes of nested messages
    #[allow(unused_variables)]
    fn compute_size(&self) -> u64 {
        let mut my_size = 0;
        if self.HBMGKJAAPFI != 0 {
            my_size += ::protobuf::rt::uint32_size(2, self.HBMGKJAAPFI);
        }
        if let Some(v) = self.BFEOOJBNFBG.as_ref() {
            let len = v.compute_size();
            my_size += 1 + ::protobuf::rt::compute_raw_varint64_size(len) + len;
        }
        if self.MAGBFEHEOOI != 0 {
            my_size += ::protobuf::rt::uint32_size(4, self.MAGBFEHEOOI);
        }
        for value in &self.FIHPPANOOFL {
            let len = value.compute_size();
            my_size += 1 + ::protobuf::rt::compute_raw_varint64_size(len) + len;
        };
        if self.NFCMAEIGKHK != 0 {
            my_size += ::protobuf::rt::uint32_size(7, self.NFCMAEIGKHK);
        }
        if self.BPBADPAACOL != 0 {
            my_size += ::protobuf::rt::uint32_size(10, self.BPBADPAACOL);
        }
        if self.LAAAPJAIKAF != 0 {
            my_size += ::protobuf::rt::uint32_size(12, self.LAAAPJAIKAF);
        }
        if let Some(v) = self.FDGKBOIEHBD.as_ref() {
            let len = v.compute_size();
            my_size += 1 + ::protobuf::rt::compute_raw_varint64_size(len) + len;
        }
        for (k, v) in &self.ODICAOCLJKO {
            let mut entry_size = 0;
            entry_size += ::protobuf::rt::uint32_size(1, *k);
            entry_size += ::protobuf::rt::uint32_size(2, *v);
            my_size += 1 + ::protobuf::rt::compute_raw_varint64_size(entry_size) + entry_size
        };
        if let Some(v) = self.FHGFEKEAJFF.as_ref() {
            let len = v.compute_size();
            my_size += 1 + ::protobuf::rt::compute_raw_varint64_size(len) + len;
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.special_fields.unknown_fields());
        self.special_fields.cached_size().set(my_size as u32);
        my_size
    }

    fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::Result<()> {
        if self.HBMGKJAAPFI != 0 {
            os.write_uint32(2, self.HBMGKJAAPFI)?;
        }
        if let Some(v) = self.BFEOOJBNFBG.as_ref() {
            ::protobuf::rt::write_message_field_with_cached_size(3, v, os)?;
        }
        if self.MAGBFEHEOOI != 0 {
            os.write_uint32(4, self.MAGBFEHEOOI)?;
        }
        for v in &self.FIHPPANOOFL {
            ::protobuf::rt::write_message_field_with_cached_size(6, v, os)?;
        };
        if self.NFCMAEIGKHK != 0 {
            os.write_uint32(7, self.NFCMAEIGKHK)?;
        }
        if self.BPBADPAACOL != 0 {
            os.write_uint32(10, self.BPBADPAACOL)?;
        }
        if self.LAAAPJAIKAF != 0 {
            os.write_uint32(12, self.LAAAPJAIKAF)?;
        }
        if let Some(v) = self.FDGKBOIEHBD.as_ref() {
            ::protobuf::rt::write_message_field_with_cached_size(13, v, os)?;
        }
        for (k, v) in &self.ODICAOCLJKO {
            let mut entry_size = 0;
            entry_size += ::protobuf::rt::uint32_size(1, *k);
            entry_size += ::protobuf::rt::uint32_size(2, *v);
            os.write_raw_varint32(114)?; // Tag.
            os.write_raw_varint32(entry_size as u32)?;
            os.write_uint32(1, *k)?;
            os.write_uint32(2, *v)?;
        };
        if let Some(v) = self.FHGFEKEAJFF.as_ref() {
            ::protobuf::rt::write_message_field_with_cached_size(15, v, os)?;
        }
        os.write_unknown_fields(self.special_fields.unknown_fields())?;
        ::std::result::Result::Ok(())
    }

    fn special_fields(&self) -> &::protobuf::SpecialFields {
        &self.special_fields
    }

    fn mut_special_fields(&mut self) -> &mut ::protobuf::SpecialFields {
        &mut self.special_fields
    }

    fn new() -> FLCJMPNDMMH {
        FLCJMPNDMMH::new()
    }

    fn clear(&mut self) {
        self.HBMGKJAAPFI = 0;
        self.BFEOOJBNFBG.clear();
        self.MAGBFEHEOOI = 0;
        self.FIHPPANOOFL.clear();
        self.NFCMAEIGKHK = 0;
        self.BPBADPAACOL = 0;
        self.LAAAPJAIKAF = 0;
        self.FDGKBOIEHBD.clear();
        self.ODICAOCLJKO.clear();
        self.FHGFEKEAJFF.clear();
        self.special_fields.clear();
    }

    fn default_instance() -> &'static FLCJMPNDMMH {
        static instance: ::protobuf::rt::Lazy<FLCJMPNDMMH> = ::protobuf::rt::Lazy::new();
        instance.get(FLCJMPNDMMH::new)
    }
}

impl ::protobuf::MessageFull for FLCJMPNDMMH {
    fn descriptor() -> ::protobuf::reflect::MessageDescriptor {
        static descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::MessageDescriptor> = ::protobuf::rt::Lazy::new();
        descriptor.get(|| file_descriptor().message_by_package_relative_name("FLCJMPNDMMH").unwrap()).clone()
    }
}

impl ::std::fmt::Display for FLCJMPNDMMH {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::protobuf::text_format::fmt(self, f)
    }
}

impl ::protobuf::reflect::ProtobufValue for FLCJMPNDMMH {
    type RuntimeType = ::protobuf::reflect::rt::RuntimeTypeMessage<Self>;
}

static file_descriptor_proto_data: &'static [u8] = b"\
    \n\x11FLCJMPNDMMH.proto\x1a\x11GLECNCOBKCJ.proto\x1a\x11NCFKHDIKCNI.prot\
    o\"\xf8\x03\n\x0bFLCJMPNDMMH\x12\x20\n\x0bHBMGKJAAPFI\x18\x02\x20\x01(\r\
    R\x0bHBMGKJAAPFI\x12.\n\x0bBFEOOJBNFBG\x18\x03\x20\x01(\x0b2\x0c.NCFKHDI\
    KCNIR\x0bBFEOOJBNFBG\x12\x20\n\x0bMAGBFEHEOOI\x18\x04\x20\x01(\rR\x0bMAG\
    BFEHEOOI\x12.\n\x0bFIHPPANOOFL\x18\x06\x20\x03(\x0b2\x0c.GLECNCOBKCJR\
    \x0bFIHPPANOOFL\x12\x20\n\x0bNFCMAEIGKHK\x18\x07\x20\x01(\rR\x0bNFCMAEIG\
    KHK\x12\x20\n\x0bBPBADPAACOL\x18\n\x20\x01(\rR\x0bBPBADPAACOL\x12\x20\n\
    \x0bLAAAPJAIKAF\x18\x0c\x20\x01(\rR\x0bLAAAPJAIKAF\x12.\n\x0bFDGKBOIEHBD\
    \x18\r\x20\x01(\x0b2\x0c.NCFKHDIKCNIR\x0bFDGKBOIEHBD\x12?\n\x0bODICAOCLJ\
    KO\x18\x0e\x20\x03(\x0b2\x1d.FLCJMPNDMMH.ODICAOCLJKOEntryR\x0bODICAOCLJK\
    O\x12.\n\x0bFHGFEKEAJFF\x18\x0f\x20\x01(\x0b2\x0c.NCFKHDIKCNIR\x0bFHGFEK\
    EAJFF\x1a>\n\x10ODICAOCLJKOEntry\x12\x10\n\x03key\x18\x01\x20\x01(\rR\
    \x03key\x12\x14\n\x05value\x18\x02\x20\x01(\rR\x05value:\x028\x01b\x06pr\
    oto3\
";

/// `FileDescriptorProto` object which was a source for this generated file
fn file_descriptor_proto() -> &'static ::protobuf::descriptor::FileDescriptorProto {
    static file_descriptor_proto_lazy: ::protobuf::rt::Lazy<::protobuf::descriptor::FileDescriptorProto> = ::protobuf::rt::Lazy::new();
    file_descriptor_proto_lazy.get(|| {
        ::protobuf::Message::parse_from_bytes(file_descriptor_proto_data).unwrap()
    })
}

/// `FileDescriptor` object which allows dynamic access to files
pub fn file_descriptor() -> &'static ::protobuf::reflect::FileDescriptor {
    static generated_file_descriptor_lazy: ::protobuf::rt::Lazy<::protobuf::reflect::GeneratedFileDescriptor> = ::protobuf::rt::Lazy::new();
    static file_descriptor: ::protobuf::rt::Lazy<::protobuf::reflect::FileDescriptor> = ::protobuf::rt::Lazy::new();
    file_descriptor.get(|| {
        let generated_file_descriptor = generated_file_descriptor_lazy.get(|| {
            let mut deps = ::std::vec::Vec::with_capacity(2);
            deps.push(super::GLECNCOBKCJ::file_descriptor().clone());
            deps.push(super::NCFKHDIKCNI::file_descriptor().clone());
            let mut messages = ::std::vec::Vec::with_capacity(1);
            messages.push(FLCJMPNDMMH::generated_message_descriptor_data());
            let mut enums = ::std::vec::Vec::with_capacity(0);
            ::protobuf::reflect::GeneratedFileDescriptor::new_generated(
                file_descriptor_proto(),
                deps,
                messages,
                enums,
            )
        });
        ::protobuf::reflect::FileDescriptor::new_generated_2(generated_file_descriptor)
    })
}
